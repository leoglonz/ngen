#ifndef NGEN_BMI_CPP_FORMULATION_TEST_CPP
#define NGEN_BMI_CPP_FORMULATION_TEST_CPP
...
class Bmi_Cpp_Formulation_Test : public ::testing::Test {
protected:
    ...
    static void call_friend_determine_model_time_offset(Bmi_Cpp_Formulation& formulation) {
        formulation.determine_model_time_offset();
    }

    static std::string get_friend_bmi_init_config(const Bmi_Cpp_Formulation& formulation) {
        return formulation.get_bmi_init_config();
    }
    static std::string get_friend_bmi_main_output_var(const Bmi_Cpp_Formulation& formulation) {
        return formulation.get_bmi_main_output_var();
    }

    static std::shared_ptr<models::bmi::Bmi_Cpp_Adapter> get_friend_bmi_model(Bmi_Cpp_Formulation& formulation) {
        return std::dynamic_pointer_cast<models::bmi::Bmi_Cpp_Adapter>(formulation.get_bmi_model());
    }

    static time_t get_friend_bmi_model_start_time_forcing_offset_s(Bmi_Cpp_Formulation& formulation) {
...
void Bmi_Cpp_Formulation_Test::SetUp() {
    testing::Test::SetUp();
...
#define EX_COUNT 2
    ...
    model_type_name[0] = "test_bmi_cpp";
    ...
    model_type_name[1] = "test_bmi_cpp";
...
void Bmi_Cpp_Formulation_Test::TearDown() {
    Test::TearDown();
}
...
TEST_F(Bmi_Cpp_Formulation_Test, Initialize_1_a) {
    Bmi_Cpp_Formulation form_1(catchment_ids[0], std::make_unique<CsvPerFeatureForcingProvider>(*forcing_params_examples[0]), utils::StreamHandler());
...
TEST_F(Bmi_Cpp_Formulation_Test, GetResponse_0_b) {
    int ex_index = 0;
...
TEST_F(Bmi_Cpp_Formulation_Test, GetResponse_1_a) {
    Bmi_Cpp_Formulation form_1(catchment_ids[0], std::make_unique<CsvPerFeatureForcingProvider>(*forcing_params_examples[0]), utils::StreamHandler());
    form_1.create_formulation(config_prop_ptree[0]);
...
TEST_F(Bmi_Cpp_Formulation_Test, GetOutputLineForTimestep_0_a) {
    int ex_index = 0;
...
TEST_F(Bmi_Cpp_Formulation_Test, GetOutputLineForTimestep_1_a) {
    int ex_index = 1;
...
TEST_F(Bmi_Cpp_Formulation_Test, GetOutputLineForTimestep_1_b) {
    int ex_index = 1;
...
TEST_F(Bmi_Cpp_Formulation_Test, determine_model_time_offset_0_a) {
    ...
    Bmi_Cpp_Formulation formulation(catchment_ids[ex_index], std::make_unique<CsvPerFeatureForcingProvider>(*forcing_params_examples[ex_index]), utils::StreamHandler());
    formulation.create_formulation(config_prop_ptree[ex_index]);
    std::shared_ptr<models::bmi::Bmi_Cpp_Adapter> model_adapter = get_friend_bmi_model(formulation);

    double model_start = model_adapter->GetStartTime();
    ASSERT_EQ(model_start, 0.0);
}

TEST_F(Bmi_Cpp_Formulation_Test, determine_model_time_offset_0_b) {
    int ex_index = 0;
...
TEST_F(Bmi_Cpp_Formulation_Test, determine_model_time_offset_0_c) {
    int ex_index = 0;
...
#endif // NGEN_BMI_CPP_FORMULATION_TEST_CPP
